...
def read_candidates(input_pattern):
  ...
  final_dict.update(read_candidates_from_one_split(input_pattern))
  return final_dict
...
def compute_predictions(example):
  ...
  summary.predicted_label = {"example_id": example.example_id,"long_answer": {"start_token": long_span.start_token_idx,"end_token": long_span.end_token_idx,"start_byte": -1,"end_byte": -1},"long_answer_score": score,"short_answers": [{"start_token": short_span.start_token_idx,"end_token": short_span.end_token_idx,"start_byte": -1,"end_byte": -1}],"short_answers_score": score,"yes_no_answer": "NONE"}
...
def main(_):
  ...
  if FLAGS.do_predict:
    ...
    for i in range(len(all_results_1)):
      start_log = [sum(j) for j in zip(all_results_1[i].start_logits, all_results_2[i].start_logits)]
      end_log = [sum(j) for j in zip(all_results_1[i].end_logits, all_results_2[i].end_logits)]
      answer_type_log =[sum(j) for j in zip(all_results_1[i].answer_type_logits, all_results_2[i].answer_type_logits)]

      start_logits = [x/2 for x in start_log]
      end_logits = [x/2 for x in end_log]
      answer_type_logits = [x/2 for x in answer_type_log]

...
def main(_):
  ...
  model_fn_1 = model_fn_builder(
      bert_config=bert_config,
      init_checkpoint=FLAGS.init_checkpoint_1,
      learning_rate=FLAGS.learning_rate,
      num_train_steps=num_train_steps,
      num_warmup_steps=num_warmup_steps,
      use_tpu=FLAGS.use_tpu,
  ...
  model_fn_2 = model_fn_builder(
      bert_config=bert_config,
      init_checkpoint=FLAGS.init_checkpoint_2,
      learning_rate=FLAGS.learning_rate,
      num_train_steps=num_train_steps,
      num_warmup_steps=num_warmup_steps,
      use_tpu=FLAGS.use_tpu,
  ...
  model_fn_3 = model_fn_builder(
      bert_config=bert_config,
      init_checkpoint=FLAGS.init_checkpoint_3,
      learning_rate=FLAGS.learning_rate,
      num_train_steps=num_train_steps,
      num_warmup_steps=num_warmup_steps,
      use_tpu=FLAGS.use_tpu,
  ...
  estimator_1 = contrib_tpu.TPUEstimator(
      use_tpu=FLAGS.use_tpu,
      model_fn=model_fn_1,
      config=run_config,
      train_batch_size=FLAGS.train_batch_size,
  ...
  estimator_2 = contrib_tpu.TPUEstimator(
      use_tpu=FLAGS.use_tpu,
      model_fn=model_fn_2,
      config=run_config,
      train_batch_size=FLAGS.train_batch_size,
  ...
  estimator_3 = contrib_tpu.TPUEstimator(
      use_tpu=FLAGS.use_tpu,
      model_fn=model_fn_3,
      config=run_config,
      train_batch_size=FLAGS.train_batch_size,
  ...
  if FLAGS.do_predict:
    ...
    for result in estimator_1.predict(
      ...
      if len(all_results_1) % 1000 == 0:
        tf.logging.info("Processing example: %d" % (len(all_results_1)))
      unique_id = int(result["unique_ids"])
      start_logits_1 = [float(x) for x in result["start_logits"].flat]
      end_logits_1 = [float(x) for x in result["end_logits"].flat]
      answer_type_logits_1 = [float(x) for x in result["answer_type_logits"].flat]

      all_results_1.append(
          RawResult(
              unique_id=unique_id,
              start_logits=start_logits_1,
              end_logits=end_logits_1,
              answer_type_logits=answer_type_logits_1))

    for result in estimator_2.predict(
        predict_input_fn, yield_single_examples=True):
      if len(all_results_2) % 1000 == 0:
        tf.logging.info("Processing example: %d" % (len(all_results_2)))
      unique_id = int(result["unique_ids"])
      start_logits_2 = [float(x) for x in result["start_logits"].flat]
      end_logits_2 = [float(x) for x in result["end_logits"].flat]
      answer_type_logits_2 = [float(x) for x in result["answer_type_logits"].flat]

      all_results_2.append(
          RawResult(
              unique_id=unique_id,
              start_logits=start_logits_2,
              end_logits=end_logits_2,
              answer_type_logits=answer_type_logits_2))

    for result in estimator_3.predict(
        predict_input_fn, yield_single_examples=True):
      if len(all_results_3) % 1000 == 0:
        tf.logging.info("Processing example: %d" % (len(all_results_3)))
      unique_id = int(result["unique_ids"])
      start_logits_3 = [float(x) for x in result["start_logits"].flat]
      end_logits_3 = [float(x) for x in result["end_logits"].flat]
      answer_type_logits_3 = [float(x) for x in result["answer_type_logits"].flat]

      all_results_3.append(
          RawResult(
              unique_id=unique_id,
              start_logits=start_logits_3,
              end_logits=end_logits_3,
              answer_type_logits=answer_type_logits_3))

    for i in range(len(all_results_1)):
      start_log = [sum(j) for j in zip(all_results_1[i].start_logits, all_results_2[i].start_logits, all_results_3[i].start_logits)]
      end_log = [sum(j) for j in zip(all_results_1[i].end_logits, all_results_2[i].end_logits, all_results_3[i].end_logits)]
      answer_type_log =[sum(j) for j in zip(all_results_1[i].answer_type_logits, all_results_2[i].answer_type_logits, all_results_3[i].answer_type_logits)]

      start_logits = [x/3 for x in start_log]
      end_logits = [x/3 for x in end_log]
      answer_type_logits = [x/3 for x in answer_type_log]

      all_results.append(
          RawResult(
              unique_id=all_results_1[i].unique_id,
              start_logits=start_logits,
              end_logits=end_logits,
              answer_type_logits=answer_type_logits))
